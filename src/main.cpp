...
int main(int argc, char **argv)
{
    ...
    // printf("is counter clockwise %d \n", is_counter_clockwise(0, 0, 0, 1, 1, 0));
    // printf("is counter clockwise %d \n", is_counter_clockwise(0, 0, 1, 0, 0, 1));
    ...
    if (argc == 2)
    {
        if (loadPath(drawsvg, argv[1]) < 0)
            exit(0);
    }
    else
    {
        msg("Usage: drawsvg <path to test file or directory>");
        exit(0);
    }
